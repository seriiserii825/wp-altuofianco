...
jQuery(document).ready(function ($) {
	...
	let showMenu = function () {
		if($(window).width() > 1265){
			$('#js-navigation > .has-children ').hover(function () {
				$('#js-navigation .sub-menu ').fadeIn(600);
			}, function () {
				$('#js-navigation .sub-menu ').hide();
			});

			$('#js-navigation .sub-menu .has-children ').hover(function () {
				$(this).find('.sub-menu-inner').fadeIn(300);
			}, function () {
				$('#js-navigation .sub-menu-inner ').hide();
			});
		}else{
			$('#js-navigation > .has-children > svg ').on('click', function () {
				$(this).next('.sub-menu').slideToggle();
			});

			$('#js-navigation .sub-menu .has-children  svg ').on('click', function () {
				$(this).next('.sub-menu-inner').slideToggle();
			});
		}
	};
	showMenu();
